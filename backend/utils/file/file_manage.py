...
class FileManage(FileBase):
    ...
    async def async_save_local(self) -> dict:
        ...
        save_path = Path(settings.STATIC_ROOT) / path
        ...
        return {
            "local_path": f"{settings.STATIC_ROOT}/{self.path}",
            "remote_path": f"{settings.STATIC_URL}/{self.path}"
        }
    ...
    async def async_save_temp_file(cls, file: UploadFile) -> str:
        ...
        await Path(temp_file_path).write_bytes(await file.read())
