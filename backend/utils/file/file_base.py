...
class FileBase:
    ...
    @classmethod
    def generate_static_file_path(cls, filepath: str, filename: str) -> str:
        """
        生成文件路径
        ...
        _filename = str(generate_string(8))+ os.path.splitext(filename)[-1]
        print(_filename)

        new_path = os.path.join(filepath ,today,_filename)
        return new_path
    @classmethod
    ...
    def generate_temp_dir_path(cls) -> str:
        ...
        file_dir = Path(settings.TEMP_DIR) / date
    ...
    async def async_generate_temp_dir_path(cls) -> str:
        ...
        """
        date = datetime.datetime.strftime(datetime.datetime.now(), "%Y%m%d")
        # print(date,"date", AsyncPath(TEMP_DIR))
        print(settings.TEMP_DIR,"Tempdie")
        file_dir = Path(settings.TEMP_DIR) / date
        path = file_dir / (generate_string(4) + str(int(datetime.datetime.now().timestamp())))
