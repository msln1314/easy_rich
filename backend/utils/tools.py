...
def find_node_and_subtree(menu_items, target_id):
    def find_node(node):
    ...
    def get_subtree_ids(node):
        ...
        if 'children' in node:
            for child in node['children']:
                ids += get_subtree_ids(child)
        return ids

    for item in menu_items:
        found_node = find_node(item)
        if found_node:
            subtree_ids = get_subtree_ids(found_node)
            return found_node, subtree_ids
