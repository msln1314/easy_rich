...
class WriteXlsx:
    ...
    def create_excel(self, file_path: str = None, sheet_name: str = "sheet1", save_static: bool = False) -> None:
        ...
        :return:
        """
        if not file_path or (file_path and not os.path.isabs(file_path)):
            if save_static:
                print(file_path,"filepath ",save_static)
                if file_path is None:
                    file_path = f'file.xlsx'

                self.file_path = FileBase.generate_static_file_path("write_xlsx", file_path)
                self.file_path = os.path.join(STATIC_ROOT, self.file_path)
        ...
        print("savepath",self.file_path)
        path_dir = os.path.dirname(self.file_path)
        print(path_dir)
        if not os.path.exists(path_dir):
            os.makedirs(path_dir,exist_ok=True)
        # save_path = Path(path_dir)
    ...
    def get_file_url(self) -> str:
        ...
        print(self.file_path,settings.STATIC_ROOT,"root",settings.STATIC_URL)
        if self.file_path.startswith(settings.STATIC_ROOT):
            return self.file_path.replace(settings.STATIC_ROOT, settings.STATIC_URL)
        else:
