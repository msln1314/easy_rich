...
async def lifespan(app: FastAPI):
    print("lifespan",settings.EVENT_LIST)
    await import_modules_async(settings.EVENT_LIST, "全局事件", app=app, status=True)
    #
    yield
    await import_modules_async(settings.EVENT_LIST, "全局事件", app=app, status=False)
...
async def connect_redis(app: FastAPI, status: bool):
    ...
    **kwargs：可选参数，其他连接参数，用于传递给 Redis 连接类的构造函数。
    ...
    if status:
        ...
        app.state.redis = aioredis.from_url(settings.REDIS_URI, decode_responses=True, health_check_interval=1)
        try:
...
async def connect_mongo(app: FastAPI, status: bool):
    ...
    motor 文档：https://motor.readtedocs.io/en/stable/
    :param app:
    :param status:
    :return:
    """
    if status:
        client: AsyncIOMotorClient = AsyncIOMotorClient(settings.MONGO_DB_URL, maxPoolSize=10, minPoolSize=10)
...
async def init_ap_scheduler(app: FastAPI, status: bool):
    ...
    if status:
        print("Initializing ap_scheduler")
        await SchedulerUtil.init_system_scheduler()

    else:
        print("ap_scheduler stopped")
