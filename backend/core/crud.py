...
class DalBase:
    ...
    async def get_count(
        ...
        :param v_outer_join: 用于创建外连接（OUTER JOIN）操作，返回两个表中满足连接条件的并集，包括未匹配的行，并用 NULL 值填充。
        :param v_where: 当前表查询条件，原始表达式
        :param v_sql: 子查询对象，作为子查询进行 count
        :param kwargs: 查询参数
        """
    

        if v_sql is not None:
            # 如果传入了 v_sql，将其作为子查询，构建 count 查询
            v_start_sql = select(func.count()).select_from(v_sql.subquery())
        else:
    ...
    def add_relation(
        ...
        :param v_options: 用于为查询添加附加选项，如预加载、延迟加载等。
        """
        print(v_select_from,"v-selaec")
        if v_select_from is not None :
    ...
    #用于非主表过滤条件返回
    @staticmethod
    def get_dict_filter(model, **kwargs) -> List[BinaryExpression]:
        """
        字典过滤
        :param model:
        :param kwargs:
