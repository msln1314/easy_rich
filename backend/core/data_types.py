...
Phone = Annotated[
    str | None,
    AfterValidator(lambda x: valid_phone(x) if x is not None else x),
    PlainSerializer(lambda x: x, return_type=str),
    WithJsonSchema({'type': 'string'}, mode='serialization')
]
...
Email = Annotated[
    str | None,
    AfterValidator(lambda x: valid_email(x) if x is not None else x),
    PlainSerializer(lambda x: x, return_type=str),
    WithJsonSchema({'type': 'string'}, mode='serialization')
]
