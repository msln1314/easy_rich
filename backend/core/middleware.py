...
from apps.admin.log.crud.operation_log import OperationLogDal
from core.database import db_getter

from apps.admin.log.models import SysOpertationLog
...
def register_operation_record_middleware(app: FastAPI):
    ...
    async def operation_record_middleware(request: Request, call_next):
        # print("kkkkkkkkkkkkk",request.scope)
        start_time = time.time()
        response = await call_next(request)
        ...
        if not phone:
            return response
        elif request.method not in settings.OPERATION_RECORD_METHOD:
            return response
        elif route.name in settings.IGNORE_OPERATION_FUNCTION:
            return response
        process_time = time.time() - start_time
        ...
        await SysOpertationLog.create_operation_log(db,document,True,request,response.__dict__)
        # await OperationLogDal(db).create_data(document)
        # print("222222222222288888888888")
        return response
