...
class SchedulerUtil:
    ...
    def add_job(cls, job_info: TaskModel) -> Job:
        ...
        try:
            ...
            if job_info.trigger == "date":
                ...
                parsed_fields = [
                    None if field in ("*", "?") else field for field in fields
                ]
                if len(fields) == 6:
                    parsed_fields.append(None)

                second, minute, hour, day, month, day_of_week, year = tuple(
                    parsed_fields
                )
                trigger = CronTrigger(
                    second=second,
    ...
    def pause_job(cls, job_id: Union[str, int]):
        ...
        """
        logger.info(
            f"开始获取全部任务：{cls.get_all_jobs()}， 状态： {cls.get_job_status()}"
        )
        query_job = cls.get_job(job_id=str(job_id))
        if not query_job:
        ...
        logger.info(
            f"查看获取全部任务：{cls.get_all_jobs()}， 状态： {cls.get_job_status()}"
        )
