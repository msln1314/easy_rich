...
async def add_review_menus():
    ...
    try:
        ...
        result = await db.execute(
            text(
                "SELECT menu_name FROM sys_menu WHERE parent_id = :parent_id AND menu_name IN ('复盘工作台', '心得经验库', '雷区基线')"
            ),
            {"parent_id": parent_id},
        )
        ...
        menus = [
            ...
            (
                ...
                "ep:notebook",
                "experience",
                "Vadmin/Stock/Review/Experience",
                max_order + 2,
            ),
            (
                "雷区基线",
