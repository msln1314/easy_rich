...
class ClientDal(DalBase):
    ...
    async def update_deal_status(self, data_id: int, status: int) -> None:
        """
        更新当前登录信息
        ...
        :return:
        """
        client = await self.get_data(data_id)
    ...
    async def create_data(
        ...
        if unique:
            raise CustomException("桩编码已存在！", code=status.HTTP_ERROR)
    ...
    async def put_data(
        ...
        :return:
        """
        options = [joinedload(self.model.device),joinedload(self.model.group)]
    ...
    async def update_bind_info(self, data_id: int, data: schemas.ClientBind) -> Any:
        """
        更新当前客户端绑定信息
        :param user:
        :param data:
        ...
        """
        options = [joinedload(self.model.device), joinedload(self.model.group)]
        exist_client_port = await self.get_data( v_options=options,v_return_none=True,v_where=[self.model.device_id == data.device_id, self.model.port == data.port])
        print(exist_client_port,"222")

        if exist_client_port :
            raise CustomException(f"设备{exist_client_port.device.mac} 端口{data.port} 已被其他充电桩：{exist_client_port.sn}绑定，请确认是否选择正确！", code=status.HTTP_ERROR)

        print('clientt1')
        client = await self.get_data(data_id, v_options=options)
        print(client,"clientt2")
        client.device_id = data.device_id
        client.port = data.port
        client.connect_type = data.connect_type
        client.connect_protocol = data.connect_protocol
        client.connect_model = data.connect_model
        client.bind_time = datetime.now()
        client.bind_status =1


        await self.flush(client)
        data = await self.out_dict(client)
        self.db.commit()
        return data

    async def update_unbind_info(self, data_id: int) -> Any:
        """
        更新当前客户端绑定信息
        :param user:
        :param data:
        :return:
        """
        data = schemas.ClientUnBind()
        data.bind_status = 0
        data.bind_time = None
        data.port = None
        data.port_status = -1
        data.device_id = None
        data.connect_model = None
        data = await self.put_data(data_id,data)
        self.db.commit()

        return data
    async def export_xlsx(self, header: list, datas) -> dict:
        """
        导出用户查询列表为 excel
        :param header:
        :param params:
        ...
        for client in datas:
            ...
            if client.get('connect_model') == '常闭' and client['port_status'] in [0, 1]:
                # print("ssssssssssssss")
                client['port_status'] = 0 if client['port_status'] == 1 else 1
            ...
            for item in header:
                ...
                if field == 'device':
                    if value:
                        value = f"mac：{value.get('mac','')} ip:{value.get('ip','')}"
                    else:
                        value = ""

                if field == "is_active":
                    value = "可用" if value else "停用"
                if field == "bind_status":
                    value = "已绑定" if value else "未绑定"
                if field == "port_status":
                    value = "合闸" if value == 1 else "断开" if value == 0 else ""


                data.append(value)
            device_info = client.get('device',"")
            online = ""
            if device_info:
                last_time = device_info.get('last_time')
        ...
        em.close()
        return {"url": file_url, "filename": "充电桩列表.xlsx"}
    ...
    async def export_query_list(self, header: list, params: ClientParams) -> dict:
        ...
        for client in datas:
            print(client.__dict__)
            if client.connect_model == '常闭' and client.port_status in [0,1]:
                client.port_status = 0 if client.port_status ==1 else 1
            data = []
            for item in header:
                field = item.get("field")
                ...
                if field == 'device':
                    if value:
                        value = f"mac：{value.mac} ip:{value.ip}"
                    else:
                        value = ""
                ...
                data.append(value)
            device_info = getattr(client,'device',"")
            online = ""
            if device_info:
                last_time = device_info.last_time

                if last_time:
                    if last_time >= offline_time:
                        online = '在线'
                    else:
                        online = '离线'
            data.append(online)
    ...
    async def delete_datas(self, ids: list[int], v_soft: bool = False, **kwargs) -> None:
        ...
        return await super(ClientDal, self).delete_datas(ids, v_soft, **kwargs)
