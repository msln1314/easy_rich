...
from .m2m import sys_user_role,sys_user_group
...
class SysUser(BaseModel):
    ...
    phone: Mapped[str] = mapped_column(String(11), nullable=True, index=True, comment="手机号", unique=False)
    ...
    last_change_pwd: Mapped[datetime | None] = mapped_column(DateTime, comment="最近一次登录时间")
    is_staff: Mapped[bool] = mapped_column(Boolean, default=False, comment="是否为工作人员")
    is_admin: Mapped[bool] = mapped_column(Boolean, default=False,nullable=True, comment="是否为管理人员")
    ...
    # iot_groups: Mapped[set['IotClientGroup']] = relationship(secondary=sys_user_group)

    dept_id: Mapped[Optional[int]] = mapped_column(ForeignKey("sys_dept.id"))
    dept: Mapped[SysDept] = relationship(back_populates="users")
    user_logs: Mapped[list["SysActionLog"]] = relationship(back_populates="user")

    data_range: Mapped[Optional[int]] = mapped_column(Integer, comment="数据范围")
    ...
    #     comment="关联部门"
    # )
    # dept: Mapped[SysDept] = relationship(foreign_keys=dept_id, back_populates="users")
    ...
    def is_super(self) -> bool:
        ...
        :return:
        """
        return self.is_admin or self.name == 'admin'
