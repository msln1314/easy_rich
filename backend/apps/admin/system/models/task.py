...
class TaskGroupModel(BaseModel):
    ...
    定时任务分组表
    """
    __tablename__ = 'sys_task_group'
    __table_args__ = ({'comment': '定时任务分组表'})
    __loader_options__ = ["tasks"]
    ...
    tasks: Mapped[Optional[list['TaskModel']]] = relationship(back_populates="task_group", lazy="selectin")
...
class TaskModel(BaseModel):
    ...
    __loader_options__ = ["task_logs", "task_group", "creator"]
    ...
    job_class: Mapped[str] = mapped_column(Text, nullable=False, comment='任务函数')
    ...
    status: Mapped[bool] = mapped_column(Boolean(), default=True, nullable=False, comment="是否启用(True:启用 False:禁用)")
    task_group_id: Mapped[Optional[int]] = mapped_column(ForeignKey('sys_task_group.id'), comment='任务分组ID')
    task_group: Mapped[Optional["TaskGroupModel"]] = relationship(back_populates="tasks", lazy="selectin")
    task_logs: Mapped[Optional[list['TaskLogModel']]] = relationship(back_populates="task", lazy="selectin")
...
class TaskLogModel(BaseModel):
    ...
    job_id: Mapped[Optional[int]] = mapped_column(ForeignKey('sys_task.id'), comment='任务ID')
    status: Mapped[bool] = mapped_column(Boolean(), default=True, nullable=False, comment="是否启用(True:启用 False:禁用)")
    # 任务关联关系
    task: Mapped[Optional["TaskModel"]] = relationship(back_populates="task_logs", lazy="selectin")