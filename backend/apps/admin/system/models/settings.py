...
class SysSettingTab(BaseModel):
    __tablename__ = "sys_settings_tab"
    ...
    status: Mapped[bool] = mapped_column(Boolean, default=True, comment="是否启用用")

    settings: Mapped[list["SysSetting"]] = relationship(back_populates="tab")
...
class SysSetting(BaseModel):
    ...
    tab_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("sys_settings_tab.id", ondelete='CASCADE'),
        comment="关联tab标签"
    )
    tab: Mapped[SysSettingTab] = relationship(foreign_keys=tab_id, back_populates="settings")
