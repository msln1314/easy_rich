...
class AuthValidation:
    ...
    def validate_token(cls, request: Request, token: str | None) -> str:
        ...
        try:
            ...
            if phone is None or is_refresh:
                raise CustomException(
                    msg="未认证，请您重新登录",
                    code=status.HTTP_403_FORBIDDEN,
                    status_code=status.HTTP_403_FORBIDDEN
    ...
    @classmethod
    async def validate_user(cls, request: Request, user: SysUser, db: AsyncSession, is_all: bool = True) -> Auth:
        """
        验证用户信息
        :param request:
        :param user:
    ...
    @classmethod
    def get_user_permissions(cls, user: SysUser) -> set:
        """
        获取员工用户所有权限列表
        :param user: 用户实例
        :return:
    ...
    @classmethod
    async def get_user_data_range(cls, user: SysUser, db: AsyncSession) -> tuple:
        """
        获取用户数据范围
