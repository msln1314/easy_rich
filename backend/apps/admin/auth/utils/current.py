...
from apps.admin.system.models import SysUser, SysRole
...
class FullAdminAuth(AuthValidation):
    ...
    async def __call__(
        ...
        if not settings.OAUTH_ENABLE:
            return Auth(db=db)
        user_id: str = self.validate_token(request, token)
        options = [joinedload(SysUser.roles).subqueryload(SysRole.menus), joinedload(SysUser.dept)]
        user = await UserDal(db).get_data(id=user_id, v_return_none=True, v_options=options, is_staff=True)
