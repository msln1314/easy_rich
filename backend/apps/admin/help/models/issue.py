...
class SysIssueCategory(BaseModel):
    __tablename__ = "sys_help_issue_category"
    ...
    issues: Mapped[list["SysIssue"]] = relationship(back_populates='category')
...
class SysIssue(BaseModel):
    __tablename__ = "sys_help_issue"
    __table_args__ = ({'comment': '常见问题记录表'})

    category_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("sys_help_issue_category.id", ondelete='CASCADE'),
        comment="类别"
    )
    category: Mapped[list["SysIssueCategory"]] = relationship(foreign_keys=category_id, back_populates='issues')
