...
async def create_stock(data: schemas.StockBaseInfoCreate, auth: Auth = Depends(AllUserAuth())):
...
async def get_stocks(params: StockBaseInfoParams = Depends(), auth: Auth = Depends(AllUserAuth())):
    stock_dict = params.dict()
    v_where = []
    if params.keywords:
        v_where.append((StockBasicInfo.full_code.like('%'+params.keywords+'%') | StockBasicInfo.name.like('%'+params.keywords+'%')))
...
async def export_query_list(
    ...
    return SuccessResponse(await crud.StockBaseInfoDal(auth.db).export_xlsx(header, datas))
...
async def put_stock(data_id: int, data: schemas.StockBaseInfoUpdate, auth: Auth = Depends(AllUserAuth())):
