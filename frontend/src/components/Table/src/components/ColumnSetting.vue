...
const initColumns = (columns: TableColumn[], isReStore = false) => {
  const newColumns = columns?.filter((item) => {
    if (!isReStore) {
      ...
      item.hidden = item.hidden !== void 0 ? item.hidden : false
    }
  ...
  const checkedLength = unref(checkColumns)?.length || 0
  const totalLength = unref(defaultCheckColumns)?.length || 0
  checkAll.value = checkedLength === totalLength
  isIndeterminate.value = checkedLength > 0 && checkedLength < totalLength
}
      ...
      <div v-if="settingColumns?.length">
        ...
        >
          ...
          >
            ...
            >
              <ElCheckbox :value="item.field" :label="item.field">
                {{ item.label }}
              </ElCheckbox>
              <div class="flex items-center">
                <ElRadioGroup size="small" v-model="item.fixed">
                  <ElRadioButton value="left" label="left">
                    <Icon icon="vi-ep:arrow-left" />
                  </ElRadioButton>
                  <ElRadioButton value="false" label="false">
                    <Icon icon="vi-ep:close" />
                  </ElRadioButton>
                  <ElRadioButton value="right" label="right">
