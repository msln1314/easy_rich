...
// 形态类型映射
export const patternTypeMap: Record<
  string,
  { name: string; signal: string; color: string; description: string }
> = {
  W_BOTTOM: {
    name: 'W底（双底）',
    signal: 'buy',
    color: '#67C23A',
    description:
      'W底是典型的底部反转形态，由两个相近的低点构成，预示行情可能由跌转涨。建议在突破颈线位后买入。'
  },
  M_TOP: {
    name: 'M头（双顶）',
    signal: 'sell',
    color: '#F56C6C',
    description:
      'M头是典型的顶部反转形态，由两个相近的高点构成，预示行情可能由涨转跌。建议在跌破颈线位后卖出。'
  },
  HEAD_SHOULDERS_TOP: {
    name: '头肩顶',
    signal: 'sell',
    color: '#F56C6C',
    description:
      '头肩顶由左肩、头部、右肩三个高点构成，是重要的顶部反转信号。建议跌破颈线后减仓或清仓。'
  },
