import { _ as a } from './Echart.vue_vue_type_script_setup_true_name_ECharts_lang-3e84099f.js'
import { e as t, o, j as i, y as s, x as e, k as r, dX as d, J as l } from './index-820a519e.js'
import './echarts-ef988edf.js'
const n = { class: 'ratio-main' },
  f = d(
    '<div class="ratio-header" data-v-26ac8af5><div class="man" data-v-26ac8af5><span data-v-26ac8af5>男士</span><img src="data:image/png;base64," alt="" data-v-26ac8af5></div><div class="woman" data-v-26ac8af5><span data-v-26ac8af5>女士</span><img src="/assets/woman-924d5660.png" alt="" data-v-26ac8af5></div></div>',
    1
  ),
  c = { class: 'echarts' },
  p = l(
    t({
      __name: 'MaleFemaleRatioChart',
      setup(t) {
        let d = 0.6,
          l = 0.4
        const p = {
          xAxis: { type: 'value', show: !1 },
          grid: { left: 0, top: '30px', bottom: 0, right: 0 },
          yAxis: [
            {
              type: 'category',
              position: 'left',
              data: ['男生'],
              axisTick: { show: !1 },
              axisLine: { show: !1 },
              axisLabel: { show: !1 }
            },
            {
              type: 'category',
              position: 'right',
              data: ['女士'],
              axisTick: { show: !1 },
              axisLine: { show: !1 },
              axisLabel: {
                show: !1,
                padding: [0, 0, 40, -60],
                fontSize: 12,
                lineHeight: 60,
                color: 'rgba(255, 255, 255, 0.9)',
                formatter: '{value}' + 100 * l + '%',
                rich: {
                  a: { color: 'transparent', lineHeight: 30, fontFamily: 'digital', fontSize: 12 }
                }
              }
            }
          ],
          series: [
            {
              type: 'bar',
              barWidth: 20,
              data: [d],
              z: 20,
              itemStyle: { borderRadius: 10, color: '#007AFE' },
              label: {
                show: !0,
                color: '#E7E8ED',
                position: 'insideLeft',
                offset: [0, -20],
                fontSize: 12,
                formatter: () => `男士 ${100 * d}%`
              }
            },
            {
              type: 'bar',
              barWidth: 20,
              data: [1],
              barGap: '-100%',
              itemStyle: { borderRadius: 10, color: '#FF4B7A' },
              label: {
                show: !0,
                color: '#E7E8ED',
                position: 'insideRight',
                offset: [0, -20],
                fontSize: 12,
                formatter: () => `女士 ${100 * l}%`
              }
            }
          ]
        }
        return (t, d) => (o(), i('div', n, [f, s('div', c, [e(r(a), { option: p, resize: !1 })])]))
      }
    }),
    [['__scopeId', 'data-v-26ac8af5']]
  )
export { p as default }
